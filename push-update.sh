...
updates_dir=/data/freakyos_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have FreakyOS_$(FREAKY_VER)-$(CUSTOM_BUILD)-$(CUSTOM_BUILD_DATE)-$(CUSTOM_BUILD_TYPE)-* as format"
...
adb shell "killall org.freaky.updater 2>/dev/null"
adb shell "sqlite3 /data/data/org.freaky.updater/databases/updates.db" \
